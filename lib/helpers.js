...
exports.chainPromises = function(promiseCreators) {
  let i = promiseCreators.length - 1
  let currPromiseCreator = promiseCreators[i] 
  while (i >= 0) {
    currPromiseCreator = new Promise(promiseCreators[i - 1]).then(currPromiseCreator) 
    i -= 1
  }
